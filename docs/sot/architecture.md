...
- 只读：仅调用语义只读的 GET endpoints；排除所有 POST 与语义上可能触发动作的 GET（例如 cancel/redeem/swap 类，以及 `/v1/spendle/{address}` 这类返回 `multiTokenProof` 的用户级 sPENDLE 奖励端点）。部分 GET 会返回报价/交易参数（如 `convert`），但本项目不签名、不广播链上交易。
- 参数约定：MCP tools 对外参数使用 snake_case；对 Pendle API 的 query/path 字段按官方命名透传（如 `chainId`、`resumeToken`、`includeFeeBreakdown`、`includeApyBreakdown`）
- `ids` 参数约定：对接 Pendle API 的 `ids`（如 assets/prices/markets）使用 `<chainId>-<address>` 作为元素格式（例如 `1-0x...`、`8453-0x...`）。
- `time_frame` 规范化：对 `get_market_historical_data_v3` / `get_prices_ohlcv_v4` 支持 `1h/1d/1w` 别名并规范化为 `hour/day/week`，非法值本地直接报错（不发起 HTTP）。
