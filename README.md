...
- **链上校准的 RPC 约束**：需要 archive RPC，跟 etherscan-mcp 一致从 `RPC_URL_<chainid>` 读。Etherscan `module=proxy` 的 eth_call 永远返回 latest（即使传历史 block tag），所以**不会** fallback 到它 —— 没配 RPC 就直接报 error。当前内置 block_time 的 chain：ETH (1)、Optimism (10)、BSC (56)、Polygon (137)、Sonic (146)、Fantom (250)、Mantle (5000)、Base (8453)、Arbitrum (42161)、Blast (81457)、Scroll (534352)。其他 chain 走 `u_actual_chain_error` 路径。
